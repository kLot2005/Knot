...
export function startBot() {
    ...
    bot.start(async (ctx) => {
        ...
        try {
            ...
            ctx.reply('Привет! Теперь ты подписан на дайджест новостей. Я буду присылать черновики прямо сюда (в личку).');
        } catch (e) {
    ...
    const start = async () => {
        try {
            await bot.launch();
            console.log('[Bot] Subscription bot is running');
        } catch (err: any) {
            console.error('[Bot] Failed to start, retrying in 10s...', err.message);
